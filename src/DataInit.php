...
// Execute "php vendor/bin/doctrine orm:schema-tool:update --force" to update the database schema
// Capture the output and errors

$output = [];
$command = "php vendor/bin/doctrine orm:schema-tool:update --complete --force 2>&1";
exec($command, $output, $returnVar);

// Display the output and return status for debugging
...
# Population Categories if they do not already exist
$categories = $jsonData['data']['categories'];

error_log(json_encode($categories));

foreach ($categories as $categoryData) {
    $category = $em->getRepository(Category::class)->findOneBy(['name' => $categoryData['name']]);
    if ($category) {
        continue;
    }
    
    $category = new Category();
    $category->populateFromJson($categoryData);
    $em->persist($category);
}

$em->flush();

# Population Products if the peoduct does not 
$products = $jsonData['data']['products'];

foreach ($products as $product){

    #Checking if the catregory exists
    $category = $em->getRepository(Category::class)->findOneBy(['name' => $product['category']]);
    if (!$category) {
        error_log("Category not found: " . $product['category']);
        continue;
    }

    #Checking if the product exists
    $productEntity = $em->getRepository(Product::class)->findOneBy(['id' => $product['id']]);
    if ($productEntity) {
        continue;
    }

    $productEntity = new Product;
    $productEntity->populateFromJson($product);
    $productEntity->flush();
}

error_log("Data initialized");
